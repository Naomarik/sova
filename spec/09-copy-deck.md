...
| Partial send banner | **Sent to {k} of {n} members.** {member} was taken by another program between the check and the send, so it didn't get this message. The {k} that did are answering now. · button `Send to {member}` — re-sends the message as it was sent, not as the box now reads · the composer keeps its text here (it clears only on a clean send) |
...
| …its failure lines | `{model} couldn't start: {server message}` — `{model}` is `shortModel(BatchRefusal.ref)`, **never** parsed out of the message; the message is the reason alone, unprefixed (162ee20). Entries sharing a ref **and** a message collapse to `{count} × {model} couldn't start: {server message}`; sharing a ref but not a message gets one line each, because the reasons are the information. **Never a member number** — `failed` carries no index into the plan, so `{model} #2` would be a guess |
| …the three shapes, worked | **one failure:** `4 of 5 members were created.` / `claude-opus-5 couldn't start: the provider returned 429.` · **two distinct refs:** `3 of 5 members were created.` / `claude-opus-5 couldn't start: the provider returned 429.` / `glm-5.3 couldn't start: no credentials for zai.` · **two sharing a ref, same reason:** `3 of 5 members were created.` / `2 × claude-opus-5 couldn't start: the provider returned 429.` · **two sharing a ref, different reasons:** two lines, the model repeated verbatim — the repetition is honest, since what differs is the reason |
