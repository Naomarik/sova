...
| Source is in an older session format | Create is `aria-disabled`, reason "“{title}” is in an older session format. Forking it means reading it, and reading it migrates the whole file — we don't rewrite a session under a runtime that has it open. Open it for chat once, then fan out." The header's `version` is already read to list the session, so this costs nothing to check and it is the one refusal the user can clear themselves in one gesture |
