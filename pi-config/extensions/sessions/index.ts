...
import type { IntercomExtensionEvent } from "./intercom.ts";
import { createPresenceChannel, type PresenceChannel, type PresenceChannelOptions } from "./presence.ts";
import { discoverFocusTarget, focusTarget, type FocusTarget } from "./focus.ts";
...
export default function sessions(pi: ExtensionAPI, deps: SessionsDeps = {}) {
  ...
  async function discover() {
    ...
    try {
      ...
      if (live) target = result;
    } catch { if (live) target = undefined; }
  ...
  function render() {
    ...
    overlay?.update(rows, connectionLabel());
    if (ctx.mode !== "tui") return;
    if (!store.connected) { ctx.ui.setStatus("sessions", "Sessions: disconnected"); return; }
    const running = rows.filter(r => /^Running/.test(r.status)).length;
    const waitingCount = rows.filter(r => r.status === "Needs input").length;
    const childCount = rows.flatMap(r => r.workers).filter(w => /^(running|starting|busy|working)$/i.test(w.status)).length;
    const unseen = rows.filter(r => r.unseen).length;
    ctx.ui.setStatus("sessions", `Sessions: ${rows.length} · ${running} running${childCount ? ` · ${childCount} workers` : ""}${waitingCount ? ` · ${waitingCount} need input` : ""}${unseen ? ` · ${unseen} unseen` : ""}`);
  }
  function snapshot(): Presence {
    const value: Presence = { type: "presence", version: 1, status, since, completed,
      preview: clean(preview, 2000), outline: outlineCache, target, workers: workers.slice(0, 40).map(w => ({
    ...
    // Presence files are re-read by every peer every couple of seconds, so
    // keep them bounded — measured in UTF-8 bytes, not JS characters.
    // The outline is enrichment: drop it before dropping workers.
    while (Buffer.byteLength(JSON.stringify(value)) > 12_000 && value.outline) delete value.outline;
    while (Buffer.byteLength(JSON.stringify(value)) > 12_000 && value.workers.length) value.workers.pop();
    return value;
  }
  function publish() {
    if (!live) return;
  ...
  function activity() {
    const next = waiting ? "Needs input" : busy ? tools.size
      ? `Running: ${[...new Set(tools.values())].join(", ")}` : "Running" : failed ? "Error" : "Idle";
  ...
  function onChannelEvent(event: IntercomExtensionEvent) {
    ...
    if (event.type === "connection") {
      ...
      store.upsert(event.session); render();
      ...
      if (payload?.type === "hello") schedule();
        ...
        if (store.peers.has(event.fromSessionId)) previous = event.fromSessionId;
        const id = selfId(); if (id) store.markSeen(id);
      } else { store.receive(event.fromSessionId, event.payload); render(); }
    }
  }
  function register() {
    ...
    channel = createChannel({
      ...
      info: () => ({
        name: clean(pi.getSessionName() ?? cwd.split("/").filter(Boolean).pop() ?? "session", 80),
        cwd,
        model: pi.getModel()?.id ?? "unknown",
        pid: process.pid,
  ...
  async function focus(id: string) {
    ...
    const peerTarget = store.target(id);
  ...
  async function open(context: ExtensionContext) {
    if (context.mode !== "tui" || overlayOpen) return;
    overlayOpen = true;
    try {
      // Do not block opening on broker I/O; the overlay updates in-place.
      void refreshRoster();
      const id = await context.ui.custom<string | undefined>((tui, theme, _keys, done) => {
        ...
        overlay.update(store.rows(), connectionLabel());
        return overlay;
      }, { overlay: true, overlayOptions: { width: "90%", anchor: "center", margin: 1 } });
      overlay = undefined;
  ...
  pi.registerCommand("sessions", { description: "Live sessions, worker status, previews and terminal switching", handler: (_args, context) => open(context) });
  pi.registerCommand("sessions-back", { description: "Focus the previous live session", handler: (_args, context) => back(context) });
  pi.registerShortcut("alt+s", { description: "Open live session switcher", handler: open });
  pi.registerShortcut("alt+shift+s", { description: "Focus previous live session", handler: back });

  pi.on("session_start", async (_event, context) => {
    ctx = context; live = true; busy = !context.isIdle(); activity();
    ...
    for (const entry of [...context.sessionManager.getBranch()].reverse()) {
      if (entry.type === "message" && entry.message.role === "assistant") {
        ...
        if (text) { preview = clean(text); break; }
      }
    }
    ...
    register();
    heartbeat = setInterval(() => { if (!channel) return; void refreshRoster(); void discover(); publish(); }, 5000);
    heartbeat.unref();
    // Pi restores its own title after session_start; apply ours after binding.
    discoveryTimer = setTimeout(() => { void discover(); }, 500);
  ...
  pi.on("agent_start", () => { busy = true; failed = false; tools.clear(); activity(); });
  pi.on("agent_settled", () => { busy = false; tools.clear(); completed = Date.now(); activity(); });
  pi.on("tool_execution_start", event => { tools.set(event.toolCallId, event.toolName); activity(); });
  pi.on("tool_execution_end", event => { tools.delete(event.toolCallId); activity(); });
  pi.on("ui_prompt_start", event => { if (!(overlayOpen && event.kind === "custom")) { waiting = true; activity(); } });
  ...
  pi.on("message_end", event => {
    ...
    if (text) preview = clean(text);
    failed = event.message.stopReason === "error";
    if (failed && event.message.errorMessage) preview = clean(event.message.errorMessage);
    schedule();
  });
}
