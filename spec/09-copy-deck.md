...
| Cancel | `Cancel` |
| Partial creation banner | **{k} of {n} members were created.** Then **one line per failure**, the refusal banner's shape (§14): `{model} couldn't start: {server message}` — `{model}` is `shortModel(BatchRefusal.ref)`, never parsed out of the message, and the message is the reason alone, unprefixed. Repeats of one model that failed the same way collapse with a count: `2 × {model} couldn't start: {server message}`; the same model failing two different ways gets two lines. **Never a member number** — `failed` has no index into the plan, so `{model} #2` would be a guess. Closing line: The {k} that exist are running; add another from Add Members. · buttons `Add Members` · `Dismiss` |
