...
| Workspace (§14) | `.app[data-view="workspace"]` `.app-workspace` `.workspace-head` `.workspace-head-main` `.workspace-title` `.workspace-meta` `.workspace-count` `.workspace-promoted` `.workspace-align` · tabs `.workspace-tabs[role=tablist]` `button.workspace-tab[role=tab]` `.workspace-tab-label` · panes `.workspace-panes` `.wpane` (+ `--wpane-w`) `.wpane-head` `.wpane-name` `.wpane-tools` `.wpane-body` `.wpane-composer` · group composer `.composer.group-composer` `.group-composer-targets` · collapsed pane composer `.composer[data-collapsed]` |
