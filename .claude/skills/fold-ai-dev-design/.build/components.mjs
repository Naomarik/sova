...
<div class="stack-2">
  ...
  <button class="button button-sm button-ghost toast-action">Undo</button>
    ...
    dos: [['Offer Undo for anything destructive','reversibility is a brand value, not a nicety'],
          ...
          ['Give a toast with an action longer, and pause it under the pointer or focus','6s against 3s, resuming with the time left, so reaching Undo never races the clock'],
          ['Replace the previous toast of the same kind','the next archive\'s Undo takes the last one\'s place, so only the latest shows']],
    donts: [['Put the only record of an error in a toast','use a banner, which stays'],
            ...
            ['Leave a toast with an action on screen until it is used','three archives became three stuck Undos (Sova overrode the earlier never-auto-dismiss rule for this)']],
  },
  {
