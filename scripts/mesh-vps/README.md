...
keys arrive by sync; `sova/peers.json` is seeded once with self id `vps`, no peers (logins of every kind sync, subscriptions included; Settings → Mesh can switch this host to API keys only); the
...
- `smoke.sh [--keep-peers]`: start Sova by hand, check health, mesh off = no peer port, PUT peers.json (the
  laptop's team server; self.id must be vps, loginKinds not pinned) → the peer listener binds the tailnet IP only, exposure probe, stop, and compare the
