...
# A user-level socat on the laptop's TAILNET IP only (100.64.0.4:4872) -> the team server 127.0.0.1:4870. No sudo, no
# `tailscale serve` (the laptop's serve config stays untouched), stopped with `stop` (pid in ~/.cache/sova-mesh/lab-engineer).
set -euo pipefail
. "$(dirname "$0")/config.sh"
LISTEN=${LAPTOP_SERVE_URL#http://}          # 100.64.0.4:4872
