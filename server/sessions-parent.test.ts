...
test("a fork's parent is its parentSession path while that file exists", async () => {
  const root = session(ROOT);
  const fork = session(FORK, root);
  const s = await getSessionSummary(fork);
  assert.equal(s?.parent, root);
});
...
test("a non-string or relative parentSession is ignored", async () => {
  const cases = [7, null, "sessions/relative.jsonl", ""];
