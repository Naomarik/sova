# §workspace/groups — Group workspaces
...
| Drag-to-resize and drag-to-reorder panes | The first cut ships `Wider` / `Narrower` and `Move Left` / `Move Right`, which are keyboard-reachable and need no pointer capture over live transcripts. §app/shell's own resizer already documents the gap a drag-only control leaves |
...
- **Nothing here pulses except work.** The tab's `.live-dot` is a running turn. A member open in
  a TUI takes `.chip-accent` with no `.chip-live`, exactly as §design/ground-rules requires.
...
- **`Wider` and `Narrower`** step that one pane's width by 120px between 440 and 1040, written to
  ...
  screen, so the first press on an auto-fitted 560px pane lands on 680 — not on 34vw. Like §app/shell's
...
`sessionStorage["sova:group-view-{id}"]` for the browser session. In tabs mode
...
(read-only, §app/session-list "Live sessions") for a member open in a TUI. Nothing about the transcript, the
...
- **A tab's visible text has to distinguish it inside this group**, which the title alone often
  ...
  `{label or title}, {model}` (§design/copy-deck) at every width.
...
- **Three ways a member leaves, and each word does one thing.** `Promote` removes it and takes
  ...
  `Eliminate` is absent rather than relabelled, because the archive half isn't available (§app/session-list
...
- **`Open`** is a link to `#/s/{path}`: the member alone, at full width, out of the workspace but
  still in the group.
- **Scroll** is the pane's own `.pane`. Jump to Latest (§chat/transcript) is per pane and sits inside it.
...
| Mid-turn | The chat, the run status row, `Steer` in its own composer (§chat/composer) | **Excluded**, reason "mid-turn". A shared prompt is not a steer |
...
| Config error | The §app/shell open-failure banner, in the pane's own banner slot, with its own actions (Reconnect · Archive, as appropriate to the diagnosis). The transcript area keeps whatever loaded | Excluded, reason "can't be opened" |
...
- **`sent` is never empty.** If not one member was accepted, that is not a partial send, it is a
  ...
  no meaning, and §design/copy-deck deliberately has no copy for it.
...
- **The refusal body is machine-readable and human-readable both**:
  ...
  **The banner is composed from `code` and the member's own name** (§design/copy-deck), never by
...
- **Attachments and slash commands are not in the group composer.** Images belong to a
  conversation (§chat/images) and a slash command is a per-runtime thing, half of them local (§chat/slash-commands). Both
...
- **Sent text appears in every included pane at once**, optimistically, as that pane's user
  bubble — the same optimistic rule §chat/composer gives.
...
the three beats §design/ground-rules requires and one row per blocked member:
...
- **Eliminate** — the same assign-to-null, plus
  ...
  turn that starts between the check and the write — where §design/copy-deck's "Removed **{title}** from
...
- **Dissolve** — `DELETE /api/session-groups/{id}`, the existing route, asked in place in the
  ...
  never touched, exactly as §app/session-list already promises. **The word differs from the sidebar's on
...
| `Esc` | Nothing new. It does not leave the workspace and it does not abort a turn (§chat/composer) |
