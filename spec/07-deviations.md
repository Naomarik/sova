...
| Horizontal scrolling and scroll snapping in `.workspace-panes` | The product's only horizontal scroller. An uncapped member count has to overflow on some axis, and one row keeps "left of" and "right of" true for the keyboard move controls; snapping is `proximity`, so two panes can still be parked half-and-half |
...
| New components: `.app-workspace`, `.workspace-*`, `.wpane*`, `.group-composer*`, `.fanout-*`, `.fork-marker`; new tokens `--wpane-min`, `--wpane-width` | Built from the skill's chat, tabs, list, chip, banner, modal, and empty patterns and from system tokens only |
