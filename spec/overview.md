...
| Timeline tab (§13) | `ol.timeline` `li.timeline-row[data-kind="input\|chapter\|marker\|density\|gap"]` (+ `.timeline-row-flagged`) `.timeline-dot` (the rail is `.timeline-row::before`) `.timeline-time` `.timeline-time-flagged` `button.timeline-body` `.timeline-title` (2-line clamp) `.timeline-chapter` (+ `.outline-hash`) `.timeline-meta` `.timeline-gap` `.timeline-state` · read-only: it jumps, and Inputs keeps the pane's only verb |
...
| [13-timeline.md](13-timeline.md) | §13 · Timeline tab | The session on one time axis: chapters, inputs, density lines, markers, and idle gaps. |
