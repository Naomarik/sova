...
## §workspace.fanout/two-sources — Two sources
...
## §workspace.fanout/cost-preview — Cost preview
...
## §workspace.fanout/the-route — The route
...
## §workspace.fanout/what-creation-does — What creation does
...
- **The source file is read, never written, and only while nobody is writing it.** Sourcing N
  ...
  **The checks that establish it add no synchronous fs call to the event loop**: a source's path
  ...
  session's `cwd`; remote sessions use local placeholders, not mounted target directories.
...
## §workspace.fanout/states — States
...
## §workspace.fanout/classes — Classes
