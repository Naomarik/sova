...
function buildVisibleBtwNote(details: BtwDetails) {
  return {
...
/**
 * Headless hosts persist each exchange once, as the displayable note (never also as a hidden
 * thread entry, or hosts that render both would show every answer twice). triggerTurn: false
 * appends now when idle and defers to the end of the current turn when busy, without starting
 * a main-agent turn the way a followUp would.
 */
function saveHeadlessBtwNote(pi: ExtensionAPI, details: BtwDetails): void {
  pi.sendMessage(buildVisibleBtwNote(details), { triggerTurn: false });
}

/**
 * One BTW exchange from a session entry: the hidden `btw-thread-entry` custom (TUI) or the
 * displayable `btw-note` custom_message (headless, or a TUI `--save`). Both carry the same details.
 */
function parseBtwExchangeEntry(entry: unknown): BtwDetails | null {
  let details: Partial<BtwDetails> | undefined;
  if (isCustomEntry(entry, BTW_ENTRY_TYPE)) {
    details = entry.data as Partial<BtwDetails> | undefined;
  } else if (
    !!entry &&
    typeof entry === "object" &&
    (entry as { type?: string }).type === "custom_message" &&
    (entry as { customType?: string }).customType === BTW_MESSAGE_TYPE
  ) {
    details = (entry as { details?: Partial<BtwDetails> }).details;
  }

  if (!details || typeof details !== "object" || !details.question || !details.answer) {
    return null;
  }
  return details as BtwDetails;
}

function btwExchangeKey(details: BtwDetails): string {
  return `${details.timestamp}\u0000${details.question}\u0000${details.answer}`;
}
...
function saveVisibleBtwNote(
  ...
  const message = buildVisibleBtwNote(details);
...
export type BtwDeps = {
  /** Sub-session factory; tests substitute a fake so no model is called. */
...
export function registerBtw(pi: ExtensionAPI, deps: BtwDeps = {}) {
  ...
  async function ensureOverlay(ctx: ExtensionCommandContext | ExtensionContext): Promise<void> {
    ...
    const shown = ctx.ui.custom<void>(
    ...
    // Headless probe. The TUI invokes the factory synchronously inside custom(), so it never
    // waits here. Headless hosts never invoke it and resolve at once; give that one macrotask.
    if (!factoryInvoked) {
      ...
      if (settled && !factoryInvoked && !runtime.handle) {
        headless = true;
        if (overlayRuntime === runtime) {
          overlayRuntime = null;
        }
  ...
  /** Headless stand-in for the overlay's input box: same submit semantics, asked via ui.input. */
  async function askHeadlessQuestion(ctx: ExtensionCommandContext): Promise<void> {
    if (!headless) {
      return;
    }

    const value = (await ctx.ui.input(getOverlayTitle(pendingMode), "Ask a side question…"))?.trim();
    if (!value) {
      return;
    }

    await submitFromOverlay(ctx, value);
  }

  /** Headless hosts show no BTW thread before a handoff, so confirm it first. TUI: always true. */
  async function confirmHeadlessHandoff(ctx: ExtensionCommandContext, title: string, message: string): Promise<boolean> {
    if (!headless) {
      return true;
    }
    ...
    if (!confirmed) {
      setOverlayStatus("Handoff cancelled. BTW thread kept.", ctx);
    }
    return confirmed;
  }
  ...
  async function dispatchBtwCommand(name: string, args: string, ctx: ExtensionCommandContext): Promise<boolean> {
    ...
    if (name === "btw:inject") {
      ...
      try {
        ...
        const exchanges = `${thread.length} exchange${thread.length === 1 ? "" : "s"}`;
        if (!(await confirmHeadlessHandoff(ctx, "Inject BTW thread?", `Send the BTW thread (${exchanges}) to the main session as a user message.`))) {
          return true;
        }
    ...
    if (name === "btw:summarize") {
      ...
      try {
        ...
        if (!(await confirmHeadlessHandoff(ctx, "Inject BTW summary?", summary))) {
          return true;
        }
  ...
  async function restoreThread(ctx: ExtensionContext): Promise<void> {
    ...
    // A TUI `--save` writes the same details as both kinds; count that exchange once.
    const seen = new Set<string>();
    for (const entry of branch.slice(lastResetIndex + 1)) {
      const details = parseBtwExchangeEntry(entry);
      if (!details || seen.has(btwExchangeKey(details))) {
        continue;
      }
      seen.add(btwExchangeKey(details));
  ...
  async function runBtw(
    ...
    try {
      ...
      if (headless) {
        // The note is the exchange's only record (see saveHeadlessBtwNote); --save is implied.
        saveHeadlessBtwNote(pi, details);
        setOverlayStatus("Ready for a follow-up. BTW note added to the session.", ctx);
        return;
      }
