---
title: Build the Sova site and docs
...
<!-- Generated by the Sova marketing playbook from .sova/marketing/brand.json and templates/doc-site.md. Edits here are replaced on the next run: change brand.json and re-run the generator (see .sova/marketing/TOOLS.md). -->

# Build the Sova site and docs
...
- **It may not claim more than the README.** Every sentence on the site must be findable in the
  ...
  knows something the README doesn't, fix the README first (**Rewrite the Sova README**).
...
Sova screenshots** first. Home uses the shot marked `"hero": true` in
