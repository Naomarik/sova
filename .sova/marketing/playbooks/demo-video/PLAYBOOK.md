---
title: Record a Sova demo video
...
<!-- Generated by the Sova marketing playbook from .sova/marketing/brand.json and templates/demo-video.md. Edits here are replaced on the next run: change brand.json and re-run the generator (see .sova/marketing/TOOLS.md). -->

# Record a Sova demo video
...
3. **Where it goes.** README and site (any length, `.webm` works), social (see the limits in
   **Write Sova announcement posts**; some platforms want `.mp4`), a release note.
