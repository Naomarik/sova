...
export function restSteps({ f, cwd, extra = [] }) {
  ...
  steps.push(
    ...
    // The Overseer (master 5e0ff37). No step here starts a model turn: /api/sessions/prompt only
    // on its refusals, and clear just rotates to a new, empty Overseer file.
    G("overseer", "/api/overseer"),
    G("overseer-attention", "/api/overseer/attention"),
    G("overseer-notes", "/api/overseer/notes"),
    J("overseer-notes-put", "PUT", "/api/overseer/notes", { text: "a parity note" }),
    J("overseer-notes-put-stale", "PUT", "/api/overseer/notes", { text: "lost", base: "not what the file holds" }),
    J("overseer-notes-put-bad", "PUT", "/api/overseer/notes", { text: 3 }),
    G("overseer-notes-after", "/api/overseer/notes"),
    G("overseer-ideas", "/api/overseer/ideas"),
    G("overseer-idea-bad", "/api/overseer/idea?id=nope"),
    G("overseer-idea-unknown", `/api/overseer/idea?id=${q("§parity/nope")}`),
    J("overseer-idea-patch-bad", "PATCH", "/api/overseer/idea?id=nope", { title: "x" }),
    J("overseer-idea-patch-unknown", "PATCH", `/api/overseer/idea?id=${q("§parity/nope")}`, { title: "x" }),
    G("settings-overseer", "/api/settings/overseer"),
    (s) => J("settings-overseer-put", "PUT", "/api/settings/overseer", s.get["settings-overseer"]?.settings),
    J("settings-overseer-put-v9", "PUT", "/api/settings/overseer", { version: 9 }), // master takes it (200): recorded, not judged
    J("overseer-clear", "POST", "/api/overseer/clear"),
    G("overseer-after-clear", "/api/overseer"),
    G("session-summary", "/api/sessions/summary?id=01970000-0000-7000-8000-000000000001"),
    G("session-summary-unknown", "/api/sessions/summary?id=01970000-0000-7000-8000-00000000dead"),
    G("session-summary-bad", `/api/sessions/summary?id=${q("../x")}`),
    J("session-prompt-bad", "POST", "/api/sessions/prompt", { path: f["real-chat"], text: 3 }),
    J("session-prompt-nopath", "POST", "/api/sessions/prompt", { text: "x" }),
    J("session-prompt-missing", "POST", "/api/sessions/prompt", { path: f["real-chat"].replace(/[^/]+$/, "missing.jsonl"), text: "x" }),
